...
public class InstallBuilder {
	...
	public InstallBuilder(String name,
	...
	public boolean createActivities() {
		...
		loop: for (DeploymentGroup deploymentGroup : deploymentGroups) {
			...
			if (containerName == null || containerName.equalsIgnoreCase("")) {
				logger
						.fine("This group has not assigned a container. This unit must be already installed in the environment");
				continue loop;
				// logger
				...
				for (Iterator<NodeType> nodeIterator = nodesList.iterator(); nodeIterator
					...
					for (Iterator<ContainerType> containerIterator = containersList
						...
						ContainerType tempContainer = (ContainerType) containerIterator
								.next();
						if (tempContainer.getName().equals(
								assignedContainerArray[i])) {
							logger
									.info("Creating activities for a group deployment to the container "
											+ assignedContainerArray[i]
											+ " : Container found");
					...
					if (container != null) {
						...
						for (Iterator<DeploymentUnit> unitIterator = units
							...
							DeploymentUnit deploymentUnit = (DeploymentUnit) unitIterator
									.next();
							logger
									.info("Creating a INSTALL deployment activity for unit "
											+ deploymentUnit.getName()
											+ " in container "
											+ assignedContainerArray[i]);
							DeploymentActivityType installActivity = PlanFactory.eINSTANCE
									.createDeploymentActivityType();
							installActivity.setActivityId(activityIndex);
							activityIndex++;
							installActivity.setTarget(environment.getName()
									+ SEPARATOR + tempNode.getName()
									+ SEPARATOR + container.getName());
							installActivity
									.setType(DeploymentActivityKindType.INSTALLDEPLOYMENTUNIT);
							installActivity.setUnitName(deploymentUnit
									.getName());
							installActivity.setUnitVersion(deploymentUnit
									.getVersion());
							// Create the start activity
							logger
									.info("Creating a START deployment activity for unit "
											+ deploymentUnit.getName()
											+ " in container "
											+ assignedContainerArray[i]);
							DeploymentActivityType startActivity = PlanFactory.eINSTANCE
									.createDeploymentActivityType();
							startActivity.setActivityId(activityIndex);
							activityIndex++;
							startActivity.setTarget(environment.getName()
									+ SEPARATOR + tempNode.getName()
									+ SEPARATOR + container.getName());
							startActivity
									.setType(DeploymentActivityKindType.STARTDEPLOYMENTUNIT);
							startActivity.setUnitName(deploymentUnit.getName());
							startActivity.setUnitVersion(deploymentUnit
									.getVersion());
							// Create the dependency between the two activities
							startActivity.getDependency().add(
									installActivity.getActivityId());
								...
								if (!preConfigure) {
									...
									for (Iterator<ResourceType> resourceIterator = resources
										...
										ResourceType resource = (ResourceType) resourceIterator
												.next();
										logger
												.info("Adding a UNITRESOURCE configuration to a install activity for unit "
														+ deploymentUnit
																.getName()
														+ "with resource"
														+ resource
